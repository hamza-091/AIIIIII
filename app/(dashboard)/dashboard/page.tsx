...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetchStats()
    // Reduced polling interval to 1 second for a more "live" feel
    const pollingInterval = setInterval(fetchLiveCall, 1000)
  ...
  useEffect(() => {
    ...
    if (liveCall && liveCall.status === "active") {
      ...
      // Reset duration when call is not active or liveCall is null
  ...
  }, [liveCall]) // Depend on liveCall to react to status changes
          ...
          <div className="flex items-center justify-between">
            ...
            {liveCall && (
              ...
              >
                {liveCall.status === "active"
                  ? "Live"
                  : liveCall.status.charAt(0).toUpperCase() + liveCall.status.slice(1)}
              </Badge>
            )}
          </div>
          {liveCall && (
        ...
        <CardContent>
          {liveCall ? (
            <div className="space-y-4 max-h-96 overflow-y-auto">
              {liveCall.transcript.length === 0 && liveCall.status === "active" ? (
                <div className="text-center py-8 text-gray-500">
                  <Phone className="h-10 w-10 mx-auto mb-3 opacity-50" />
                  <p className="font-medium">Call connected, waiting for speech...</p>
                  <p className="text-sm">Transcript will appear here as the conversation unfolds.</p>
                </div>
              ) : (
              ...
              {liveCall.status === "active" && liveCall.transcript.length > 0 && (
